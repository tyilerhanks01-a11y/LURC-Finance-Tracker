...
- `supabase-migration-approval-notifications.sql` — one-time migration that emails a user (via Resend's API) the moment an admin approves them out of `pending`. Reuses the same Vault secret as the signup-notification migration above.
