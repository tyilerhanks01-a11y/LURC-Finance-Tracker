...
-- Email a user (via Resend) the moment an admin approves them out of pending.
-- Doesn't need to query profiles at all (everything's on the row already),
-- so it isn't exposed to the search_path issue notify_pending_signup() hit.
